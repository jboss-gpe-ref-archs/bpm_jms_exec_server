...
.. Add the following to the list of dependecies: `<module name="org.acme.insurance" export="true" services="import" meta-inf="import"/>`
...
[start=3]
. In your local command window, Use a comand, structured like the following to make a local clone of your remote repository on the BPM Suite 6 server:
...
. Take note of the deployment Id in the column labled `Deployment`.
